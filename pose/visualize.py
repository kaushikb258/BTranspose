...
def inspect_atten_map_by_locations(
    ...
    # KAUSHIK ADDED THE FOLLOWING 3 LINES
    ...
    # KAUSHIK CHANGED THIS ON 01/24/21

    #feature_hooks = [
    #    model.model_alpha.reduce.register_forward_hook(
    #        lambda self, input, output1: features.append(output1)
    #    )
    #]
    ...
    # KAUSHIK: USE THE FOLLOWING FOR GETTING ATTENTION DEPENDENCIES 
    atten_maps_hooks = [
        model.model_alpha.global_encoder.layers[i].self_attn.register_forward_hook(
            lambda self, input, output: global_enc_atten_maps.append(output[1])
        )
        for i in range(len(model.model_alpha.global_encoder.layers))
    ]
    
    # len(atten_maps_hooks) = 4 
    ...
    # len(features) = 1
    # features[0].shape = torch.Size([1, 256, 32, 24])
    
    shape = [32, 24] #features[0].shape[-2:] #[32, 24]
    enc_atten_maps_hwhw = []
    for atten_map in global_enc_atten_maps:
        # atten_map.shape = torch.Size([1, 768, 768])
        atten_map = atten_map.reshape(shape + shape)
        # atten_map.shape = torch.Size([32, 24, 32, 24])
    ...
    # KAUSHIK CHANGED THIS TO GET RID OF RANDOM
    fig, axs = plt.subplots(attn_layers_num, 20-2, figsize=(27, 8),)
    fig.subplots_adjust(
...
def inspect_atten_map_by_locations_bt(
    image,
    model,
    query_locations,
    model_name="transposer",
    ...
    atten_maps_hooks = [
        model.model_alpha.layer4[-1].conv2[0].softmax.register_forward_hook(
            lambda self, input, output: global_enc_atten_maps.append(output[-1]) 
        )     
    ]
    ...
    for atten_map in global_enc_atten_maps:
        for i in range(4):
            amap = atten_map[i].reshape(shape + shape)
            enc_atten_maps_hwhw.append(amap)
    ...
    for l in range(attn_layers_num):
        axs[l][0].imshow(img_vis.copy()) #(img_vis_kpts)
        axs[l][0].set_ylabel("MHSA Att.\nLayer {}".format(l), fontsize=25)
        axs[l][0].set_xticks([])
        axs[l][0].set_yticks([])
...
# THE FOLLOWING IS A DUMMY FUNCTION NOT IN USE 
 
def inspect_atten_map_by_locations_bt2_DO_NOT_USE(
    image,
    model,
    query_locations,
    model_name="transposer",
    ...
    enc_atten_maps_hwhw = []
    for atten_map in global_enc_atten_maps:
        # atten_map.size() = torch.Size([4, 768, 768])
        atten_map = torch.reshape(atten_map[0], (32, 24, 32, 24)) 
        enc_atten_maps_hwhw.append(atten_map)
        
    
    # shape = torch.Size([32, 24])    
    ...
    fig, axs = plt.subplots(nrows, ncols, figsize=(28, 14),)
    fig.subplots_adjust(
        bottom=0.07, right=0.97, top=0.98, left=0.03, wspace=0.00008, hspace=0.02,
    )

    axs[0,0].imshow(img_vis_kpts)
    axs[0,0].set_xticks([])
    axs[0,0].set_yticks([])

    # len(enc_atten_maps_hwhw) = 1
    ...
    for id, attn_map in enumerate(enc_atten_maps_hwhw):
        for p_id, p in enumerate(attn_map_pos):
            ...
            amin = np.amin(attention_map_for_this_point)
            attention_map_for_this_point = np.log(attention_map_for_this_point - amin + 1e-5)
            
            x, y = p[0] * down_rate, p[1] * down_rate
            img_vis_kpts_new = img_vis.copy()
            #axs[row_id, col_id].imshow(img_vis_kpts_new)
            ...
            #    mask = attention_map_for_this_point <= threshold
            #    attention_map_for_this_point[mask] = 0
            #    im = axs[row_id, col_id].imshow(attention_map_for_this_point, aspect='equal')
            #else:
            im = axs[row_id, col_id].imshow(attention_map_for_this_point, aspect='equal')
